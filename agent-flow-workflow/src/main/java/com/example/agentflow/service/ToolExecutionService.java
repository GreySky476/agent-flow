...
public class ToolExecutionService {
    ...
    public String execute(String toolName, Map<String, Object> parameters) {
        ...
        return switch (entry.getType()) {
            ...
            case MCP -> "{\"error\": \"MCP not supported in feature-v1\"}";
        };
    }
    ...
    private Object resolveParameter(Parameter param, Map<String, Object> parameters) {
        return parameters.getOrDefault(param.getName(),
                parameters.getOrDefault("query",
                parameters.getOrDefault("input",
                parameters.values().stream().filter(v -> v != null).findFirst().orElse(null))));
    }
